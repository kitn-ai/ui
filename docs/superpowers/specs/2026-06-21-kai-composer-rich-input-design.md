...
**Fixed during prototype hardening (2026-06-22):** placeholder reappears after clearing (drop trailing filler `<br>`); triggers fire immediately adjacent to a pill (text walkers skip pill-internal label text via `createTextWalker`); the menu excludes entities already added (same kind+id); textarea-parity events (`kai-focus`/`blur`/`focusin`/`focusout`/`keydown`/`paste`); **undo/redo** via a doc-snapshot history (`composer-history.ts`) replacing the broken native contenteditable undo (Cmd/Ctrl+Z, Shift+Z / Ctrl+Y; typing coalesced, structural edits = own steps).

**Known v1 limitations (deferred):** the `highlights` prop recomputes on input / value-change / mount, but changing ONLY the `highlights` prop (no value change) doesn't re-run; per-rule highlight `class` not yet wired (single default highlight); declarative `<kai-trigger>` light-DOM children not read (prop-driven `triggers` only); undo caret restoration is best-effort at pill boundaries (content is exact); `Composer` is now a large file (~700 lines) — extract the trigger-menu, caret, and history wiring when promoting past prototype.
