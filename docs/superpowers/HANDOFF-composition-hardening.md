...
## 0.0 LATEST (2026-06-26) — ★INTERACTION-API ROLLOUT in flight (Wave 3 UNCOMMITTED). Read the memory running log first.
**★READ memory `composition-seams-direction.md` — cont. 38 is the authoritative current state + the Wave-3 RESUME CHECKLIST.** Everything below in this doc is older history; trust the memory.

**Where we are:** the composition→docs→Storybook arc (the original subject of this doc, §0.5 onward) is DONE + committed on `docs/composition-promotion` (= **PR #111**, still open, Rob review/merge pending). On top of it, a NEW initiative is mid-rollout: **give every interactive `kai-*` element its full INTERACTION API — events (out) + methods (in) + helpers** — using the **lightweight Shoelace/WebAwesome model** (settable+reflecting `open` + `defaultOpen` + `kai-open-change` + `show`/`hide`/`toggle` + `disabled` for overlays; CONTROLLED `value`/`checked` only for form-controls; `disabled` everywhere; explicitly NOT React-controlled — Rob's call, the web-component-idiomatic path).

**Foundation (committed):** `src/elements/define.tsx` gained `ctx.expose(methods)` (instance-method mechanism, via defineProperty); `src/elements/disclosure.ts` `wireDisclosure(ctx, getApi, openProp)` is the reusable overlay helper; `gen-element-api.mjs` recognizes helper-provided methods (`HELPER_METHODS`). SPEC: `docs/superpowers/specs/2026-06-26-interaction-api-audit.md` (Part 1 latent audit + Part 2 Radix/Shoelace benchmark; prioritized into 4 waves). AGENT GUIDE: `docs/superpowers/plans/2026-06-26-interaction-api-rollout-guide.md` (Pattern A overlays/B imperative/C controllerRef + the parallel-agent workflow).

**Process that's working:** dispatch background agents (same cwd, DISJOINT files, concurrency-aware) to implement element source from the guide+helper; then I CONSOLIDATE per wave — `build:api` (+ `git checkout -- src/components/component-meta.json`) → 4-pass `npm run typecheck` → restart Storybook → **browser IVP per element** (Playwright; elements have no Storybook stories now, so `document.createElement('kai-x')` in any kit iframe like `labs-foundations--icons`, or use a Test-Fixtures story) → unit/drift/`build-storybook` → ONE `feat(elements)` commit.

**Committed so far (11 elements):** chat+prompt-input (`062105c`), hover-card (`859290e`, helper refactor `192bddf`), tooltip (`4d01055`), popover/menu/button/command (`6765663`), message/conversations/switch (`a4f7903`).

**★IMMEDIATE NEXT (resume Wave 3 — IMPLEMENTED but UNCOMMITTED; 12 files in the working tree):** elements `kai-tool`, `kai-reasoning`, `kai-workspace`, `kai-composer`, `kai-model-switcher`, `kai-scope-picker` — all agent-implemented, build:api done, **4-pass tsc CLEAN**, **IVP 4/6 PASS** (tool/reasoning/model-switcher/scope-picker). FINISH: (1) IVP `kai-composer` (story `test-fixtures-composer`) + `kai-workspace`; (2) regression-check message (collapsibles agent edited `src/components/tool.tsx`+`reasoning.tsx`, imported by `message.tsx`); (3) build-storybook + unit/drift gate; (4) commit Wave 3 as one `feat(elements)`. THEN: kai-chain-of-thought (accordion: value/defaultValue/kai-value-change) · WAVE 4 cards (choice/confirm/form/tasks/cards/artifact/compare — card-contract, Pattern C) · voice-input (start/stop; getUserMedia IVP is hard → mock/defer) · then **render methods into the doc surfaces** (gen-web-components-md + gen-llms + kai-MCP `component_reference` + a docs-site MethodTable.astro — methods currently land in element-meta + CEM only).

**Open flags:** kai-switch cross-shadow `<form>` submit needs a `formAssociated`/ElementInternals pass in `define.tsx` (deferred); many untracked `_*.mjs`/`_wave*.mjs` IVP scratch helpers in repo root — DON'T commit (stage explicitly); prompt-input `.shot` baselines swept into `5204cda`; PR #111 still awaits Rob's review/merge.
