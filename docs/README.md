...
**Most recent session:** [`handoff/2026-09-21-serverinfo-instructions-and-two-guards.md`](handoff/2026-09-21-serverinfo-instructions-and-two-guards.md) — the three follow-up items after #382 (the CLI's own version in the MCP `instructions`, the `lint:dangling-imports` guard, kai's pack guard), the two things CI caught that the local ladder could not (a colon in a workflow `name:`; a fresh checkout has no build products), and why §3.2's CLI consolidation needs its dependency edge pointed the other way.
