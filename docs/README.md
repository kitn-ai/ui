...
**Session close (start here after a reset):** [`handoff/2026-09-19-session-close.md`](handoff/2026-09-19-session-close.md) — the state of `update/primitives`, every decision the owner made with the reasoning, the verified ladder, the open work, and the traps. **Most recent session:** [`handoff/2026-09-20-invariant-floor-imports.md`](handoff/2026-09-20-invariant-floor-imports.md) — it supersedes that doc's whole §7: §7.1 the acceptance floor resolves a snippet's kit imports, so the invariant catalog imports the real URL predicates instead of copying the scheme lists; §7.2 the audit's remaining sink-coverage vectors are pinned; §7.3 the flake is pinned and now self-naming; §7.4 the leftover wording is swept and the nested-`d.ts` question measured and declined.
