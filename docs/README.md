...
**Most recent session:** [`handoff/2026-09-25-sweep-lands-release-and-two-story-bugs.md`](handoff/2026-09-25-sweep-lands-release-and-two-story-bugs.md) — the sweep's stack merged in order, `@kitn.ai/ui 0.37.0` published and verified against the tarballs (not the tree), the Kbd-weld merge resolved against the sweep, a seeded-open modal no longer covers its own Autodocs page (#414), and `Components/Message/Narrow Panel` collapsed to one story plus a new rule (o) that bans an HTML tag or a code fence in a RENDERED description — the bug where a story's Source panel rendered inside its own blurb. Previous session: [`handoff/2026-09-24-verbosity-sweep-passes.md`](handoff/2026-09-24-verbosity-sweep-passes.md) — the whole sweep, passes A to E, each with its guard: the Lightbox PR merged (`878e0999`), four stacked PRs (#410-#413) waiting on the merge order, four new or widened guards (`lint-prop-docs` at four rules, `lint:comment-references`, `docs-copy-concepts.test.ts`, `lint-story-conventions` (l) widened and (n)), the traps (a lane editing the guard, a fork-context lane believing it is the orchestrator, a paragraph split that dropped words, and a text-identity proof blind to code fences) and the remaining follow-ups. The Kbd weld (`<kai-kbd-group>` welding its caps, the tooltip taking rich content) is recorded in [`handoff/2026-09-22-kbd-weld-and-tooltip-content.md`](handoff/2026-09-22-kbd-weld-and-tooltip-content.md). Previous session: [`handoff/2026-09-22-verbosity-sweep.md`](handoff/2026-09-22-verbosity-sweep.md) — the verbosity sweep: one rule (a description is ONE plain statement of what the component is or does, and it never teaches) applied to four gated surfaces (prop docs, element one-liners, component descriptions, page tops) plus a fifth found late (a story's hand-written argType descriptions, which win in Storybook), with a guard per surface, the `copy-reviewer` agent that reads the criteria file, and the pass-by-pass plan for what is left. The rules and counters live in [`verbosity-sweep.md`](verbosity-sweep.md). Previous session: [`handoff/2026-09-22-lightbox-component.md`](handoff/2026-09-22-lightbox-component.md) — the Lightbox: a standalone `Lightbox` trio in Solid plus `<kai-lightbox>` (default slot = trigger, `slot="content"` = media, `kai-open-change`, `show()`/`hide()`/`toggle()`, `show-close`, `close-on-content-click`, the kit's Dialog for the modal contract), and the attachments family composing it behind an opt-in (`image-preview="lightbox"` on `kai-attachments`, `imagePreview` on `Attachments`/`MessageBody`/`ChatThread`/`Thread`). Same session, later: `Image` split into a resource (`Image`, `src`) and a payload (`ImageArtifact`, `data` + a REQUIRED `mediaType`), which is why the generated API went to 100 web components. Plus the recorded reasoning (why not `GeneratedImage`, why not one polymorphic prop), the traps (a story name shadowing an import, two Solid components rendering MessageBody, `Uint8Array` vs `BlobPart`, an em dash visible only after regeneration), and the `llms-full.txt` ceiling raise with its note. (The Kbd weld iteration rode its own branch, PR #408: see `handoff/2026-09-22-kbd-weld-and-tooltip-content.md` once that lands.) Previous session: [`handoff/2026-09-22-storybook-and-component-sweep.md`](handoff/2026-09-22-storybook-and-component-sweep.md) — the twelve Storybook and component items the owner reported, and the four root causes behind most of them: docgen's exact-string control inference (so every optional primitive rendered an object control), conventions nobody guarded (events, autodocs, self-contained snippets, hand-rolled glyphs, now four lint rules), `position: fixed` not escaping a containing block (so a floating layer portals or is clipped), and silent fallbacks. Plus `KbdGroup`, the deferred Storybook SECTION question with the corrected criterion, and the traps. Previous session: [`handoff/2026-09-22-per-tag-scaffold-imports.md`](handoff/2026-09-22-per-tag-scaffold-imports.md) — the scaffold and the starters now register only the `kai-*` tags they place: the React wrappers already self-registered per tag (so the react starter's barrel import was pure redundancy), the raw-tag apps take one per-tag entry each loaded dynamically (measured: a static block put 624.7 kB in the vue entry chunk against 100.8 kB dynamic), the SSR-capable targets keep the browser guard because a per-tag entry is client-only, and `verify:scaffold`'s new derived check asserts the block per surface. Previous session: [`handoff/2026-09-22-release-shipped-and-the-skip-path.md`](handoff/2026-09-22-release-shipped-and-the-skip-path.md) — the queued release shipped (`create-kai` 0.8.0, `@kitn.ai/cli` 0.4.0) and the failed publish it uncovered first: a skipped publish runs no `prepublishOnly`, so the kit's `dist/` never existed and the later packages' builds resolved `@kitn.ai/ui/<subpath>` into it. The pre-loop build step and the derived guard that now assert that, the artefacts driven from the registry rather than read, and the traps (npm 404s for minutes after a successful publish; a benign `bin` normalization warning worth checking against the tarball; a `ci:` commit inside a package's directory bumping nothing). Previous session: [`handoff/2026-09-22-release-automation-and-cli-verbs.md`](handoff/2026-09-22-release-automation-and-cli-verbs.md) — the two releases (0.34.0 and 0.35.0) and the lockfile automation that made the second one routine, the five new guards, the CLI's four doors (`create`/`add`/`init`/`upgrade` plus `doctor` and `mcp`), the `kai.json` baseline `upgrade` and `doctor` both read, the design corrections a test found, the traps (a `GITHUB_TOKEN` push triggers no workflows; a `: ` in a workflow scalar breaks the file) and the ranked open work. Previous session: [`handoff/2026-09-21-serverinfo-instructions-and-two-guards.md`](handoff/2026-09-21-serverinfo-instructions-and-two-guards.md) — the three follow-up items after #382 (the CLI's own version in the MCP `instructions`, the `lint:dangling-imports` guard, kai's pack guard), the two things CI caught that the local ladder could not (a colon in a workflow `name:`; a fresh checkout has no build products), and why §3.2's CLI consolidation needs its dependency edge pointed the other way.
