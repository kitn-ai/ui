...
|---|---|---|
| React (Vite) | ✅ works | wrappers, per-web-component lazy registration |
...
**Yes.** `import '@kitn.ai/ui/web-components/chat'` self-registers just `kai-chat`. The React wrappers do this per-component (lazy import of `@kitn.ai/ui/web-components/<name>` on first client mount), so a `<Chat>` consumer ships Chat + its chunk, not all 78. Verified in every example.

### 2. Is it SSR-safe?
**Yes.** Registration is gated to the browser and fires only in a client effect; on the server `<kai-*>` render as inert tags that upgrade after client registration. `import '@kitn.ai/ui/web-components'` imports cleanly in pure Node (no `window` touched). Next.js + TanStack Start both SSR + hydrate with **zero hydration mismatches**.

### 3. Will my bundler tree-shake unused components?
**At runtime, yes** — only the web components you actually render get fetched (a `Button`+`Conversations` page loads ~109 kB, not all 78). Two caveats: the coarse `@kitn.ai/ui/web-components` is intentionally "register everything" (side-effectful, not tree-shakeable — by design); and the React wrapper *barrel* still **emits** a chunk per web component in the build output even when unused (no `/*#__PURE__*/` on the factory calls) — runtime is unaffected, but for minimal build output import web components directly (`@kitn.ai/ui/web-components/button`). *(Known follow-up: PURE-annotate the wrapper factories.)*
...
- Toast only, lean: `import { toast } from '@kitn.ai/ui'` (main entry). *(Known gap: the convenience `toast` re-export on `@kitn.ai/ui/web-components` isn't typed — works at runtime, type-errors; use the main entry.)*
- Everything: `import '@kitn.ai/ui/web-components'` (side-effectful, registers all).
