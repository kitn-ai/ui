...
- A store only earns its place if we later want elements to **auto-coordinate without host wiring** (e.g. a `<kitn-chat-provider>` ancestor that children read). That reintroduces implicit global coupling + multi-instance ambiguity, so it's **deferred** — add opt-in only on a concrete need. SSR is explicitly out of scope.
...
|---|---|---|---|
| `<kitn-chat>` | (composes many) | ✅ | batteries-included |
| `<kitn-conversation-list>` | conversation-list | ✅ | |
| `<kitn-prompt-input>` | prompt-input + default-input | ✅ | |
| `<kitn-thinking-bar>` | thinking-bar | 🔬 | |
| `<kitn-model-switcher>` | model-switcher | 🔬 | |
| `<kitn-attachments>` | attachments | 🔬 | variant+flags exemplar |
| `<kitn-message>` | message (+ reasoning/tool/attachments/actions) | 📋 | **data-driven single message** — the keystone |
| `<kitn-markdown>` | markdown | 📋 | |
| `<kitn-code-block>` | code-block | 📋 | |
| `<kitn-reasoning>` | reasoning | 📋 | |
| `<kitn-tool>` | tool | 📋 | |
| `<kitn-context-meter>` | context | 📋 | |
| `<kitn-chain-of-thought>` | chain-of-thought | 📋 | needs a `steps` data shape |
| `<kitn-source>` / `<kitn-source-list>` | source | 📋 | |
| `<kitn-loader>` | loader | 📋 | 12 variants |
| `<kitn-feedback-bar>` | feedback-bar | 📋 | |
| `<kitn-prompt-suggestions>` | prompt-suggestion | 📋 | list form |
| `<kitn-slash-command>` | slash-command | 📋 | overlay/portal |
| `<kitn-file-upload>` | file-upload | 📋 | dropzone + slot |
| `<kitn-voice-input>` | voice-input | 📋 | **special: function property** |
| `<kitn-image>` | image | 📋 | |
| `<kitn-text-shimmer>` | text-shimmer | 📋 | |
| `<kitn-checkpoint>` | checkpoint | 📋 | |
| `<kitn-chat-scope-picker>` | chat-scope-picker | 📋 | |
| `<kitn-message-skills>` | message-skills | 📋 | |
| `<kitn-response-stream>` | response-stream | 📋 | property-only data |
| `<kitn-empty>` | empty | 📋 | slot-based |
| `<kitn-chat-container>` | chat-container | 📋 | optional; slot + scroll |
...
⤵️ `MessageContent`/`MessageAvatar`/`MessageActions`/`MessageCopyButton` → inside `<kitn-message>`. `CodeBlockCode`/`Group` → inside `<kitn-markdown>`/`<kitn-code-block>`. `ConversationItem` → inside `<kitn-conversation-list>`. `ScrollButton`, `ChatContainerContent`/`ScrollAnchor` → context-bound, live inside `<kitn-chat>`/`<kitn-chat-container>` (a standalone `<kitn-scroll-button>` can't see another element's scroll context).
...
#### `<kitn-message>` — a single message row (keystone element)
Data-driven equivalent of one row inside `<kitn-chat>`. Renders markdown/plain content, reasoning, tool calls, attachments, and action buttons from one object.
...
#### `<kitn-context-meter>` — token-usage meter
...
#### `<kitn-chat-scope-picker>` — author/tag filter
...
#### `<kitn-voice-input>` — mic capture + transcription ⚠️ special case
...
#### `<kitn-text-shimmer>` — animated shimmering text
- **Attributes:** `text` (or default slot), `duration` (number), `spread` (number).

#### `<kitn-image>` — base64 / bytes image with skeleton fallback
- **Properties:** **`base64: string`** or **`bytes: Uint8Array`**, `alt` (attr), `media-type` (attr).

#### `<kitn-checkpoint>` — bookmark/checkpoint button
- **Attributes:** `variant`, `size`, `tooltip`.
- **Events:** `select` (no detail).

#### `<kitn-message-skills>` — active-skill badges
- **Properties:** **`skills: { id, name }[]`**.

#### `<kitn-source>` / `<kitn-source-list>` — citation link(s) + hover preview
- `<kitn-source>` **Attributes:** `href`, `label`, `title`, `description`, `show-favicon` (flag).
- `<kitn-source-list>` **Properties:** **`sources: { href, title, description }[]`**.

#### `<kitn-response-stream>` — animated streaming text
...
#### `<kitn-empty>` — empty-state layout
- **Attributes:** `empty-title`, `description`.
- **Slots:** `media` (icon/illustration), default slot (actions).

#### `<kitn-chain-of-thought>` — step-by-step reasoning ⚠️ needs a data shape
...
#### `<kitn-chat-container>` — scroll-aware message viewport
For hosts building a fully custom stream who still want the scroll-to-bottom behavior. Internally owns its scroll context (so it can include its own scroll button — a standalone `<kitn-scroll-button>` can't reach across elements).
...
1. **`<kitn-message>` shape:** single `message` object property (recommended — matches `<kitn-chat>`) vs. flattened props + slots. Pick one as canonical.
2. **`<kitn-chain-of-thought>` step model:** lock the `steps[]` schema (above) — it's the one element with no natural data prop today.
3. **Function-valued properties** (`<kitn-voice-input>.transcribe`): confirm the convention and document it as a first-class pattern, not an exception.
4. **Pluralization:** list-style elements (`<kitn-prompt-suggestions>`, `<kitn-source-list>`) vs. singular item elements. Recommend list-form for anything you'd `For`-loop.
...
- **Phase 1 — message core:** `<kitn-message>`, `<kitn-markdown>`, `<kitn-code-block>`, `<kitn-reasoning>`, `<kitn-tool>`. Unlocks "compose your own message list in plain HTML." Highest value.
- **Phase 2 — header/meta:** `<kitn-context-meter>`, `<kitn-chat-scope-picker>`, `<kitn-feedback-bar>` (+ `<kitn-model-switcher>` 🔬).
- **Phase 3 — input:** `<kitn-prompt-suggestions>`, `<kitn-slash-command>`, `<kitn-file-upload>`, `<kitn-voice-input>`.
