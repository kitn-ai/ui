...
- ✅ **`:root` token override now reaches the shadow DOM** — Tailwind v4 pinned `@theme` tokens on `:host`, which shadowed any inherited `:root` override (affected `--color-*` too). `define.tsx` now strips `:host` from the shadow token rule (`SHADOW_CSS`) and injects defaults onto the document `:root` in a low-priority `@layer kitn-tokens`, so elements still self-theme AND a consumer's `:root { --color-…/--text-… }` override wins. Verified for colors + typography, light + dark. Trade-off: generic token names now inherit from the host `:root` (namespacing is the escape hatch).
