...
- ✅ **Themeable via a namespaced `--kitn-*` surface (isolated + overridable)** — Tailwind v4 pins `@theme` tokens on `:host`, which (good) isolates the kit from a host's generic `--color-primary`/`--radius` but (bad) blocked `:root` overrides. Fix: each kit token in `theme.css` is now defined as `--color-x: var(--kitn-color-x, <default>)` (light + dark, colours + `--radius` + `--text-*`; done via a regex pass). So the `:host` pin stays (isolation — a shadcn host's `--color-primary` can't bleed in), but the fallback resolves the **namespaced** `--kitn-color-x` which inherits from `:root` → consumers override `--kitn-color-*`/`--kitn-text-*`/`--kitn-radius`. No utility renames (utilities still read `--color-x`). Single `--kitn-*` value applies to both modes; scope to `:root.dark` for per-mode. Verified: host `--color-primary` ignored, `--kitn-color-primary`/`--kitn-text-meta` apply inside the shadow, self-theming intact light+dark. (Superseded the earlier de-pin+inject approach.)
