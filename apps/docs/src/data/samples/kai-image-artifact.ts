// Sample data for <kai-image-artifact> — the AI-PAYLOAD renderer.
//
// `data`, `alt`, and `mediaType` are scalar props but need to be seeded here so
// the Playground and bare Examples render a meaningful image rather than a blank
// skeleton. This follows the same pattern as kai-code-block's `code` prop.
//
// `data` also accepts a `Uint8Array`, which cannot be serialised in a static data
// file — set it imperatively: `el.data = new Uint8Array([...])`.
//
// A string here is BARE base64, never a URI: that is what the online providers
// hand back. A `data:` URI belongs on <kai-image>'s `src`.
//
...
export default {
  sample: {
    data: toBase64(STAR_SVG),
    alt: 'A purple star icon',
    mediaType: 'image/svg+xml',
  },
  named: {
    circle: {
      data: toBase64(CIRCLE_SVG),
      alt: 'A teal circle icon',
      mediaType: 'image/svg+xml',
    },
    checkmark: {
      data: toBase64(CHECK_SVG),
