...
On mount, `<kai-remote>` validates `provider-origin`, injects a sandboxed `<iframe>` pointing at `src`, completes a version-negotiated handshake, then pushes the `CardEnvelope` + `CardContext` (theme, locale) down the wire. The provider renders the card using its own renderers; user interactions come back as `CardEvent` messages.

On the host side every event re-surfaces as a bubbling `kai-card` `CustomEvent` so your existing listeners work unchanged.
...
Register the web components and give the component its three required pieces — `provider-origin`, `src`, and the `envelope` property:
...
The kit exports this type. Import it instead of restating it:

```ts
import type { CardEnvelope } from '@kitn.ai/ui';

const envelope: CardEnvelope = {
  type: 'booking',                // maps to a renderer type on the provider
  id: 'bk_1',                     // stable across re-renders — correlates every CardEvent
  title: 'Confirm your booking',  // optional heading
  data: { slot: '2026-01-05' },   // passed verbatim to the renderer's mount()
  resolution: { kind: 'submit', data: { slot: '2026-01-05' } }, // freezes the card resolved
};
```

`resolution` is a `CardResolution`, which has **four** kinds — `action`, `submit`,
`dismissed` and `expired`, each with an optional `at` timestamp.
...
- **[`kai-remote` reference](/components/remote/)** — all props, origin validation rules, and the security model in full.
- **[`kai-cards` reference](/components/cards/)** — the card dispatcher: built-in types, `types` overrides, and `CardPolicy`.
- **[Drop-in chat](/examples/drop-in-chat/)** — the streaming loop that drives `cards` into a `<kai-chat>` message thread.
- **[Generative UI cards](/patterns/generative-ui-cards/)** — render the same cards locally, in-thread.
