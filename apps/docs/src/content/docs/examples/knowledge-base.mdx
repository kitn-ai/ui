...
- **`<kai-sources numbered>`** shows the citations behind that answer — a separate element below the chat, because the retriever owns them. Citations that arrive as `source` parts on a message render their own row inside the thread instead.

`<kai-resizable>` wraps the sidebar and main pane so the user can drag the split.

### Wire the scope to the data

The picker offers tags; map each tag to a knowledge base, then refresh the dependent web components when it changes.
...
<script type="module">
  ...
  chat.addEventListener('kai-submit', async (e) => {
    ...
    chat.messages = [
      ...chat.messages,
      { id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text: e.detail.value }] },
