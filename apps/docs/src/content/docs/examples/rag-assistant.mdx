...
This example keeps the row separate because the retriever, not the model, owns the citations; sources that arrive as `source` parts on a message render their own row inside the thread.
