...
Border radii are controlled by `--radius` (default `0.6rem`); `--radius-sm`, `--radius-md`, `--radius-lg`, and `--radius-xl` derive from it. Those four are Tailwind's own radius names, so in a Tailwind build that imports `theme.css` they also become what `rounded-sm` through `rounded-xl` mean. See [what theme.css changes](#what-themecss-changes-in-a-tailwind-build).
