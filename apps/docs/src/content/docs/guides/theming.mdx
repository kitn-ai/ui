...
Each element adopts the kit's compiled CSS into its own shadow root, and that sheet declares every `--color-*` on `:host` as `var(--kai-color-<name>, <default>)`. A declaration on the host beats a value inherited from the page, so overriding **`--color-primary` on `:root` does not retheme a `kai-*` element** — importing `theme.css` doesn't change that.

`--kai-color-*` is the layer the web components read.
...
Those are Tailwind's own radius names, so in a Tailwind build that imports `theme.css` they also become what `rounded-sm` through `rounded-3xl` and `rounded-pill` mean. See [what theme.css changes](#what-themecss-changes-in-a-tailwind-build).
...
If you import `theme.css` into your own Tailwind build, the re-point applies to **your** `text-sm` too. Deliberate: kit and app move together on one scale. It does mean a `--kai-text-*` override is not sandboxed to `kai-*` web components.

Import `theme.tokens.css` instead, or neither, if that is not what you want; the full list of what `theme.css` changes is [above](#what-themecss-changes-in-a-tailwind-build).
</Aside>
