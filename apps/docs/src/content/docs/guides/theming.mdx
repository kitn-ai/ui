...
<Aside type="caution" title="`--kai-color-*` is the token that reaches the web components">
Each element adopts the kit's compiled CSS into its own shadow root, and that sheet declares every `--color-*` on `:host` as `var(--kai-color-<name>, <default>)`. A declaration on the host beats a value inherited from the page, so overriding **`--color-primary` on `:root` does not retheme a `kai-*` element** — importing `theme.css` doesn't change that. `--kai-color-*` is the layer the web components read.
...
The kit injects its own scoped CSS into each component's Shadow DOM. The `kai-*` web components need no import at all, and the lightest way to retheme them is the [`--kai-color-*` tokens](#namespaced-overrides---kai-color-) with no stylesheet.
...
Set your overrides on `:root` **after** importing `theme.css`. This is the host-page layer — your own markup and the light-DOM Solid components. To retheme the `kai-*` web components, use the [`--kai-color-*` form](#namespaced-overrides---kai-color-) instead; every token below has one.
...
The kit never adds `.dark` to your `<html>` — your page's chrome is yours. It only syncs the web components, each of which follows `prefers-color-scheme` on its own while `theme` is `auto`. Pin an element with `theme="light"` / `theme="dark"` when your app owns the choice, and mirror the media query onto your own markup:
...
If you import `theme.css` into your own Tailwind build, the re-point applies to **your** `text-sm` too. Deliberate: kit and app move together on one scale. It does mean a `--kai-text-*` override is not sandboxed to `kai-*` web components. Import `theme.tokens.css` instead, or neither, if that is not what you want; the full list of what `theme.css` changes is [above](#what-themecss-changes-in-a-tailwind-build).
</Aside>
