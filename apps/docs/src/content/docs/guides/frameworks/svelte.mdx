...
Register every `kai-*` web component once, near your app entry point. Registration is async, and Svelte assigns object properties as it mounts, so mount behind `webComponentsReady`. A property written before the element upgrades is discarded.
...
webComponentsReady.then(() => mount(App, { target: document.getElementById('app') }));
```
