---
title: React
description: Use AI/UI's typed React wrappers to drop kai-* web components into any React app with idiomatic JSX, typed props, and onX event handlers.
---
...
import '@kitn.ai/ui/web-components'; // registers all kai-* web components globally
```
...
Every web component has its own wrapper, so you can assemble your own layout. This example pairs a `<Conversations>` sidebar with a `<Chat>` thread. `groups` carries the section headers and `conversations` the rows; pass `groups={[]}` and everything lands in one Ungrouped section.
...
Drop individual web components anywhere in your UI without adopting a full chat shell — `<Markdown>`, `<CodeBlock>`, `<Reasoning>`, `<Tool>`, `<Artifact>` all work standalone:
...
setConversations((prev) => { prev.find((c) => c.id === id)!.title = 'Renamed'; return [...prev]; });

// Renders: a new array, and the one item that changed is a new object.
setConversations((prev) => prev.map((c) => (c.id === id ? { ...c, title: 'Renamed' } : c)));
```

This is the same immutable update React already wants for its own state, so idiomatic React code satisfies it by default. The trap is reaching for a mutation to avoid a re-render, or handing the web components state from a store that mutates in place.

<Aside type="tip" title="Image name collision">
`kai-image` exports as `Image`, which shadows the browser global. Alias it on import: `import { Image as KaiImage } from '@kitn.ai/ui/react'`. The payload element exports as `ImageArtifact`, which collides with nothing, so it needs no alias.
</Aside>
