...
<p class="kai-lede">AI/UI web components are custom elements with Shadow DOM, so they hydrate and run in the browser. In Next.js and TanStack Start that makes them client islands: the page renders on the server, and `kai-chat` comes alive where it mounts.</p>

## Next.js (App Router)

Register the web components in the browser, then render the tag. The cleanest path is a client component that imports `@kitn.ai/ui/web-components` in an effect, so the import never runs during the server pass.
...
export default function ChatIsland() {
  ...
  useEffect(() => {
    import('@kitn.ai/ui/web-components').then(() => {
...
Same idea, different gates. Wrap the chat in `ClientOnly` from `@tanstack/react-router` so it mounts only after hydration, and register the web components in an effect:
...
function Chat() {
  const ref = useRef<HTMLElement>(null);
  useEffect(() => { import('@kitn.ai/ui/web-components'); }, []);
  return <kai-chat ref={ref} style={{ display: 'block', height: '600px' }} />;
}
...
<Chat
  messages={messages}
  onSubmit={(e) => setMessages((m) => [...m, { id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text: e.detail.value }] }])}
...
The same web components run in a desktop shell. An Electron or Tauri webview loads the bundle and the `@kitn.ai/ui/web-components` import upgrades the tags identically — no separate build, no different API.
