...
Registration is **async**. If your framework sets array or object properties as it mounts (Vue, Angular, Svelte all do), mount behind the `webComponentsReady` promise the same entry exports. A property written before the element upgrades is lost.

```ts
import { webComponentsReady } from '@kitn.ai/ui/web-components';

webComponentsReady.then(() => mountYourApp());
```
