...
The web components are controlled: you own the data, pass it in, listen for events. To make
...
// Append a user turn
setMessages((m) => appendMessage(m, { id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text }] }));

// Patch a field on an existing message (e.g. correct the text after an edit)
setMessages((m) => updateMessage(m, id, { parts: [{ type: 'text', text: corrected }] }));

// Insert-or-replace by id (useful for streaming placeholders)
setMessages((m) => upsertMessage(m, { id, role: 'assistant', parts: [{ type: 'text', text: partial }] }));

// Append a delta onto an existing message's trailing text part (streaming primitive)
setMessages((m) => appendText(m, id, delta));
```
...
| Method | Purpose |
|---|---|
| `appendText(chunk)` | Append a text delta to the trailing text part |
| `appendReasoning(chunk, opts?)` | Append a reasoning delta (`opts` can set `label`, provider `index`, or `signature`) |
| `upsertTool(toolCallId, patch)` | Insert a tool part (first call) or merge `patch` onto its `ToolPart` (later calls), keyed by `toolCallId` |
| `addCard(envelope)` | Insert a generative-UI card part (first call) or replace it in place on later calls, keyed by `envelope.id` |
| `addSource(source)` | Append a citation part |
| `addFile(attachment)` | Append a file/attachment part |
| `done()` | Mark the message complete |
| `abort(reason?)` | Mark the message aborted; flips any in-flight tool parts to `output-error` |

Re-send a card with the same `envelope.id` to revise it in place — a card can fill in as the model works instead of stacking duplicates.
...
el.addEventListener('kai-submit', async (e) => {
  ...
  set((m) => appendMessage(m, { id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text: e.detail.value }] }));
