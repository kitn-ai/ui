...
An unversioned CDN URL tracks the latest release, which is risky on a pre-1.0 package. Pin an exact version: `https://cdn.jsdelivr.net/npm/@kitn.ai/ui@0.26.0/dist/elements/autoloader.js`.
...
| Strategy | SSR-safe? |
|---|---|
| `@kitn.ai/ui/elements` | Yes — registration is gated on `window`; no `document` on import |
| Per-element (`@kitn.ai/ui/elements/chat`) | No — throws `window is not defined`; use inside a client-only boundary |
