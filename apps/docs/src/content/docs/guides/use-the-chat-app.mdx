...
This guide assumes you've installed the package and registered the web components. If not, do the [Getting Started](/guides/getting-started/) quickstart first, then come back here to go deeper.
...
- `messages` is an array of `{ id, role, parts }`. Every entry needs a stable `id` (it keys the render and message actions); `role` is `'user'` or `'assistant'`; `parts` is an ordered array of `text` / `reasoning` / `tool` / `card` / `source` / `file` parts. Optional fields — `actions`, `avatar` — light up the matching UI when present.
...
`<kai-chat>` is transport-agnostic: it renders whatever you hand it, so it doesn't care whether tokens come from your own backend, a proxy, or a mock. You own the `fetch`; the kit parses what comes back.
...
chat.addEventListener('kai-submit', async (e) => {
  ...
  // 2. Stream from your endpoint. createAssistantStream appends the assistant
  //    message and hands readOpenAIStream a sink to fold deltas onto.
...
`loading` toggles the streaming indicator and disables submit while the reply is in flight. Set it back to `false` when the stream ends. For the tool loop, Anthropic streams and the error model, see the [wire adapter recipe](/guides/recipes/wire-adapter/); for `kai-response-stream` and the raw-frames path, the [streaming recipe](/guides/recipes/streaming/).
...
- **`kai-web-search`** / **`kai-voice`** — the toolbar Globe and Mic buttons, shown when you set the `webSearch` and `voice` props. Each fires with an empty detail; you own what happens next. (Renamed from `kai-search`/`search`: the old names now belong to conversation filtering, and there is no alias.)
...
export function App() {
  ...
  return (
    <Chat
      ...
      onSubmit={(e) => {
        const turn: ChatMessage = { id: crypto.randomUUID(), role: 'user', parts: [{ type: 'text', text: e.detail.value }] };
