...
description: Lay out your own message list from standalone kai-message web components — one per turn — and wire its per-message slots for a header, a footer, and the avatar.
---
...
const turns = [
  { id: 'm-u', role: 'user', parts: [{ type: 'text', text: 'Which model handled this, and what did it cost?' }] },
  {
    id: 'm-a',
    role: 'assistant',
    parts: [{ type: 'text', text: 'Here is the summary you asked for. The citation is below.' }],
