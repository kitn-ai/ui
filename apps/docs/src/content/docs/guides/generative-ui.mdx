...
`artifact` reaches a deliberately narrow slice of the `<kai-artifact>` element: an envelope sets `src`, `files`, `tab`, `activeFile`, `displayUrl` and `height`, never the iframe `sandbox`, the toolbar composition or the view state. A model says what to show; the host keeps control of how the viewer behaves.

`tab` and `activeFile` seed the first render only, so revising the card leaves a user who has since switched views where they are.
