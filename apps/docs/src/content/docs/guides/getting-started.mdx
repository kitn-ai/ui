...
Drop `<kai-chat>` into a page, wire two things, and you have a live chat UI. This guide goes from install to a web component that handles real submissions.
...
One import registers every `kai-*` web component in the browser's custom element registry. Run it once — at the top of your entry file, or in a `<script type="module">`:
...
Pin an exact version in production (e.g. `@kitn.ai/ui@0.37.0/dist/...`).
...
This bundle registers everything in one fetch — right for a chat page. A page placing only a web component or two can load the [autoloader](/guides/loading/#autoloader) instead, which fetches each web component on demand.
</Aside>
...
- **[Components](/components/chat/)** — full props, events, and examples for `kai-chat` and every other web component.
