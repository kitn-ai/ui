...
```bash
copilot mcp add kai -- npx -y @kitn.ai/ui mcp
```
...
dsh takes MCP servers as a cordis plugin — `@deepseek-ai/dsh-mcp-client`, one plugin instance per server, wired through a `cordis.patch.yml` patch with a `transport: stdio` config. dsh is still in developer preview and its plugin config keys are moving, so rather than paste a snippet that might already be stale, check dsh's own MCP docs (Settings → MCP in the app, or its `docs/` tree) for the current format. Whatever the surrounding config looks like, the server underneath is unchanged: a stdio process launched as `npx -y @kitn.ai/ui mcp`.
