...
dsh takes MCP servers as a cordis plugin — `@deepseek-ai/dsh-mcp-client`, one plugin instance per server, wired through a `cordis.patch.yml` patch with a `transport: stdio` config. dsh is still in developer preview and its plugin config keys are moving, so rather than paste a snippet that might already be stale, check dsh's own MCP docs (Settings → MCP in the app, or its `docs/` tree) for the current format. Whatever the surrounding config looks like, the server underneath is unchanged: a stdio process launched as `npx -y @kitn.ai/mcp`.

</TabItem>
<TabItem label="Pi">

Pi's core has **no MCP support**, by design. Its own README puts it plainly: "**No MCP.** Build CLI
tools with READMEs (see Skills), or build an extension that adds MCP support." So there is no config
file to paste. Two routes do work:

**Use the CLI as a tool.** That is the shape Pi recommends, and it needs no integration at all — Pi
runs shell commands, so `npx -y @kitn.ai/cli add support-widget`, `npx -y @kitn.ai/cli doctor` and
`npx -y @kitn.ai/cli validate widget.construct.json` work as they are. Paste
[`llms.txt`](#machine-readable-files) into the session (or point at `llms-full.txt`) for the API
reference.

**Or add MCP yourself** through an [extension or a Pi package](https://github.com/earendil-works/pi)
that speaks the protocol, and the server underneath is the same one: a stdio process launched as
`npx -y @kitn.ai/mcp`.

</TabItem>
<TabItem label="Hermes">

Hermes ships MCP support in the standard install. In its `config.yaml`:
...
Or let its CLI write the same entry, and check it:
...
`/reload-mcp` picks up a config change without restarting. Hermes names the tools
`mcp__kai__<tool>`.
