...
There is **no axe-core rule sweep in CI**, and the kit does not currently pass one cleanly. A bare mount of every web component reports, under the `wcag2a / wcag2aa / wcag21a / wcag21aa` tags, three rules: `button-name` (3 nodes), `label` (1) and `role-img-alt` (1).

Widening to axe's **best-practice** rules adds `empty-heading` — `<kai-settings-group>` renders its `<h3>` whether or not it has a label. Which rule set you run matters, so it is named here rather than implied.
