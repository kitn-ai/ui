...
<p class="kai-lede">A passive panel slot that carries layout config for its enclosing `<kai-resizable>` — set `size`, `min`, `max`, `locked`, or `hidden` as attributes and the parent handles the rest.</p>
