---
title: Compare
description: Shows two responses and records which one the user prefers.
---
...
<p class="kai-lede">Collects a preference pair from a single pick, for evaluation or feedback data.</p>
...
When you want to capture which of two answers a user prefers: A/B collection, RLHF-style feedback, a regenerate-and-compare flow. The pick is a one-shot commit, not a form submit.
</Aside>

## Try it

<Playground client:only="solid" tag="kai-compare" />
...
- **`data`**: `{ prompt?, candidates: [A, B], collapse? }`. Each candidate renders exactly like a [message](/components/message/) body (markdown, reasoning, tool calls, attachments).
- **`kai-compare-select`** — fires once when the user picks. `detail` is `{ chosenId, rejectedIds, at }`.
- The pick **commits and collapses** to the chosen candidate, and `<kai-compare>` won't emit again for the same definition.
...
The selection is the whole point. `kai-compare-select` hands you `chosenId` and `rejectedIds` (for a pair, the other candidate's id); pair that with the prompt and send it wherever you collect preference data:
...
Both responses can stream at once: mark a candidate `streaming: true` and its column shows a shimmer, while the pick controls stay disabled until **both** settle and `kai-ready` fires.
...
To show a comparison the user already answered (loading a conversation), set `selection`: the card renders collapsed to the chosen candidate and emits nothing.
...
The columns are a WAI-ARIA radiogroup with roving focus: Arrow keys move between A and B, Enter or Space picks the focused one.
