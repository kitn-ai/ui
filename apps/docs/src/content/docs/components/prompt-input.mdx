...
- **Scalar flags** — `placeholder`, `disabled`, `loading`, `webSearch` (attribute `web-search`), `voice`, `stoppable`, and `suggestionMode` work as plain attributes or properties.
...
`webSearch` and `voice` flags add Globe and Mic buttons to the toolbar, firing `kai-web-search` and `kai-voice` respectively. (Renamed from `search`/`kai-search`: those names now belong to conversation filtering, and there is no alias.)
