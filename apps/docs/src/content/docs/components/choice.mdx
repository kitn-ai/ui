---
title: Choice
description: A card that offers options and reports the one the user picks.
---

import { Aside } from '@astrojs/starlight/components';
import Playground from '../../../components/Playground.tsx';
import Example from '../../../components/Example.tsx';
import PropTable from '../../../components/PropTable.astro';
import MethodTable from '../../../components/MethodTable.astro';
import ComposedFrom from '../../../components/ComposedFrom.astro';
import Facts from '../../../components/Facts.astro';
import NeedAHand from '../../../components/NeedAHand.astro';

<p class="kai-tag-sub">kai-choice</p>

<p class="kai-lede">Resolves to a read-only summary once the pick is in, so the thread keeps its record.</p>

<Facts items={['Shadow DOM', 'Card-contract events', 'Rich option rows', 'Optional free-text escape (allowOther)', 'Resolves to read-only after submit']} />

## Preview

<Playground client:only="solid" tag="kai-choice" />

<Aside type="tip" title="When to use">
When the model needs one answer before it continues: a plan, a tone, a branch in the next step. For yes or no, use [Confirm](/components/confirm/).
</Aside>

## Usage

Set `data` in JavaScript (it's an object) and listen for the `kai-card` CustomEvent:

```html
<kai-choice id="ch" heading="Choose a plan"></kai-choice>

<script type="module">
  import '@kitn.ai/ui/web-components';

  await customElements.whenDefined('kai-choice');

  const el = document.getElementById('ch');

  el.data = {
    prompt: 'Which plan fits your team?',
    options: [
      { id: 'free',  label: 'Free',     meta: '$0' },
      { id: 'pro',   label: 'Pro',      meta: '$12/seat', recommended: true },
      { id: 'biz',   label: 'Business', meta: '$29/seat' },
    ],
  };

  el.addEventListener('kai-card', (e) => {
    const ev = e.detail; // CardEvent
    if (ev.kind === 'action') console.log('chose:', ev.action, ev.payload);
  });
</script>
```

- **`data`** — required fields per option: `id` and `label`. Optional: `description`, `meta`, `recommended`, `media` + `imageAlt`, `disabled`, `payload`.
- **`allowOther`** appends an "Other…" row revealing an inline text input; submits as `action: '__other__'` with `payload: { text }`.
- **After submit** the card renders read-only. Restore a resolved state on reload via `el.resolution = { kind: 'action', action: 'option-id' }`.

## Examples

### Plans with Pricing

<Example client:visible tag="kai-choice" config={{ heading: 'Choose a plan', cardId: 'card-plans' }} />

### Rows with Images

<Example client:visible tag="kai-choice" config={{ heading: 'New workspace', cardId: 'card-templates' }} data="templates" />

### Quick Replies with Free-Text Escape

`allowOther` appends an "Other…" row that reveals an inline text field when selected.

<Example client:visible tag="kai-choice" config={{ cardId: 'card-replies' }} data="quickReplies" />

### Resolved (Read-only)

Pre-render the post-submit view — useful for restoring persisted choices on reload.

<Example client:visible tag="kai-choice" config={{ heading: 'Choose a plan', cardId: 'card-resolved' }} data="resolved" />

### Error State

A malformed definition (empty `options`) triggers the inline error view and emits `{ kind: 'error' }`.

<Example client:visible tag="kai-choice" config={{ cardId: 'card-error' }} data="error" />

## Props

<PropTable tag="kai-choice" />

## Methods

<MethodTable tag="kai-choice" />

## Composed from

<ComposedFrom tag="kai-choice" />

<NeedAHand />
