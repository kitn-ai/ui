...
description: Reveals text a piece at a time as it arrives.
---
...
<p class="kai-lede">Takes a plain string or an async iterable, so it can animate a reply that is still coming.</p>
...
For a reply that streams in live, so the reader watches it grow instead of waiting. For a finished string in prose, use [Markdown](/components/markdown/).
</Aside>
