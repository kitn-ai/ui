 ...
 * its own next-step command (`npx -y @kitn.ai/cli dev ...`) then rejected: no
...
export async function emitConstruct(
  ...
  return {
    file,
    devCommand: `npx -y @kitn.ai/cli dev ${fileName}`,
 ...
 * inherited so the user sees the same live-reload server `npx -y @kitn.ai/cli
