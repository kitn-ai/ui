#!/usr/bin/env node
/**
 * GUARD: the CLI's tarball is the shape a release expects: its two bundles, the two dev pages and
 * the two dispatcher files are IN it, nothing from node_modules is, and it has not changed size by
 * a STEP. (`@kitn.ai/mcp` has its own copy of this guard, because the two packages ship separately
 * and the MCP's tarball is the one carrying the SDK.)
 *
 * WHY THIS EXISTS. `packages/ui/scripts/verify-pack-weight.mjs` is ui-specific and this package
 * ships its own tarball, which until this guard existed had a ceiling only by inspection. The
 ...
 * `dist/builder-page/assets/*` at 261,583 B. The ceilings below are tripwires for an order-of-magnitude
...
export const LIMITS = {
  packedCeiling: 1_572_864, // 1.5 MiB; measured 211,038 B
  packedFloor: 100_000,
  unpackedCeiling: 4_194_304, // 4 MiB; measured 787,480 B
  unpackedFloor: 400_000,
  fileCeiling: 2_097_152, // 2 MiB; measured largest 261,583 B (the builder page bundle)
};
...
if (process.argv.includes('--self-test')) {
  const HEALTHY = [
    { path: 'bin/kai.js', size: 4_800 },
    { path: 'bin/route.js', size: 2_400 },
    { path: 'dist/doctor.es.js', size: 12_000 },
  ...
  const probes = [
    ...
    [
      ...
      packProblems(healthyEntry, big('dist/construct-cli.es.js', LIMITS.fileCeiling + 1)).some((p) => p.includes('per-file ceiling')),
    ],
    [
    ...
    [
      ...
      (() => {
        const found = packProblems(healthyEntry, [
          ...drop('dist/doctor.es.js'),
          { path: 'dist/node_modules/zod/index.js', size: 10 },
        ]);
        return found.some((p) => p.includes('dist/doctor.es.js is MISSING')) && found.some((p) => p.includes('node_modules'));
      })(),
    ],
  ];
  let failed = 0;
...
if (!existsSync(join(PKG, 'dist', 'doctor.es.js'))) {
  console.error(`✗ verify-pack-weight: ${join(PKG, 'dist', 'doctor.es.js')} is missing. Run \`npm run build\` in packages/cli first.`);
  process.exit(1);
}
