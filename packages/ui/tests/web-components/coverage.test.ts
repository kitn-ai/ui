/**
 * GUARD — every `kai-*` web component in the manifest is exercised by SOME test, or is a
 * NAMED exemption carrying a reason that says what is missing.
 *
 * WHY THIS FILE EXISTS. A coverage audit of the 79 manifest tags found web components with
 ...
 *
 *   (a) a test file IMPORTS the web component's facade module — resolved through the real
 *       filesystem from the manifest's own tag→module mapping, so a rename cannot
 *       leave a string behind that still matches — or CONSTRUCTS the web component, which
...
const ELEMENTS = join(PKG, 'src/web-components');
 ...
 * React layer by construction — a wrapper test imports `@kitn.ai/ui/web-components` by
...
function testFiles(): string[] {
  return [...walk(join(PKG, 'src')), ...walk(join(PKG, 'tests'))]
    ...
    // the web components it exempts, which is how it was caught: the stale-exemption test
    // named kai-dialog, kai-pane and kai-pane-group as "now covered by
    // coverage.test.ts". A guard's own fixtures are not coverage.
 ...
 * they can be dropped.
 *
 * Type-only imports are the second overcounting trap: a test that imports a web
 * component's PROP TYPE compiles against it without executing a line of it.
...
export function importsOf(source: string): Imported[] {
  ...
  // and how web-component-registry.test.ts drives its late-definition case.
...
describe('element coverage', () => {
  it('the scan is not vacuous', () => {
    ...
    expect(files.some((f) => f.endsWith('chat.test.tsx'))).toBe(true);
  ...
  it('the Solid-component derivation really derives something', () => {
    ...
    expect([...solidModulesOf('kai-dialog')]).toEqual([join(PKG, 'src/components/dialog/dialog.tsx')]);
  ...
  it('rejects tag mentions that are not construction — THE TEETH', () => {
    ...
    expect(importsOf(`import type { DockProps } from '../../src/web-components/dock';`)[0].typeOnly).toBe(true);
  });
  ...
  it('rejects tags inside REGEX LITERALS — the hole that hid kai-tool', () => {
    ...
    expect(
      constructsTag(`const p = join(PKG, '/src/web-components'); el = document.createElement('kai-badge');`, 'kai-badge'),
  ...
  it('accepts real construction', () => {
    ...
    expect(constructsTag(`render(() => <kai-chat messages={m} />);`, 'kai-chat')).toBe(true);
    expect(constructsTag(`document.createElement<HTMLElement>('kai-pane')`, 'kai-pane')).toBe(true);
    expect(importsOf(`import '../../src/web-components/badge';`)[0].typeOnly).toBe(false);
  ...
  it('every manifest element is exercised by a test, or is a named exemption', () => {
    ...
    expect(
      ...
      + 'tests/web-components/ that mounts the element (document.createElement(tag) or JSX), '
