...
${jsxTagMap}
  ...
  // differed on exactly that: src/web-components/web-component-types.d.ts
  // imports ../primitives/highlighter and ../components/tool-classify, which
  // reach solid-js, so types/jsx.d.ts was in the program and the bare name
  // resolved; the shipped dist/web-components.d.ts is deliberately
  // self-contained (no imports at all), so
   ...
   *  dist/web-components.d.ts, and any consumer whose program has no other
   *  reason to load solid's JSX types) leaves this \`declare module\` a fresh
   *  ambient declaration instead of a merge, and the bare name is TS2694 under
   *  \`skipLibCheck: false\`. See scripts/gen-web-component-types.mjs. */
