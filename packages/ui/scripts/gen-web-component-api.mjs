...
// the facades in src/web-components/, using the TypeScript compiler API, and emits:
//   - custom-elements.json          (Custom Elements Manifest, for IDE/tooling)
//   - src/web-components/jsx-types.d.ts   (HTMLElementTagNameMap + per-element types)
...
// See writeTypes() in gen-web-component-types.mjs for what is done with this, and
// tests/web-components/types-lib-check.test.ts for the guard that proves it.
...
if (DOM_MEMBERS.size === 0) throw new Error('gen-web-component-api: could not resolve HTMLElement members from lib.dom.d.ts');
...
// web-component-meta.json, web-component-types.d.ts, the React wrappers, llms-full.txt,
...
function unhandledKind({ site, construct, node, kindOf = 'member', hint }) {
  ...
  const where = sf
    ...
    : '<unknown source file>';
  throw new Error(
    `gen-web-component-api (${site}): unhandled ${kindOf} kind ${ts.SyntaxKind[node.kind]} in ${construct}\n` +
      `  at ${where}\n` +
...
function defaultsFrom(objLiteralNode, tag) {
  ...
  if (!objLiteralNode || !ts.isObjectLiteralExpression(objLiteralNode)) {
    throw new Error(
      `gen-web-component-api (defaultsFrom): defineWebComponent('${tag}') passes ` +
 ...
 *  where they got a `solid-advanced-web-components-defaultwarnthreshold--docs` story id
...
const composedImports = (sourceFile, seen = new Set([sourceFile.fileName])) => {
  ...
  for (const st of sourceFile.statements) {
    ...
    // Single-valued since src/ui/ was merged into src/components/: a facade's
...
// ---- the props EVERY web component gets, injected by defineWebComponent ----
...
// web component — but it was declared nowhere in the model, and instead re-typed by
// hand in three places (the .d.ts propBody, the CEM `attributes` list, the React
// `propNames` array) and absent entirely from web-component-meta.json, llms-full.txt
...
const UNIVERSAL_PROPS = (() => {
  ...
  if (!out.length) throw new Error('gen-web-component-api: the `defaults` literal in define.tsx declares no universal props');
  return out;
})();

// The few web components with web-component-specific tokens; everything else is themed by
...
// Several web components accept a "Route 2" child API next to their JS property —
...
// web components from plain HTML with no JS at all, they are unit-tested
// (*.declarative.test.tsx) and documented in the facades' JSDoc — and they were in
// NO generated artifact: not web-component-meta.json, not custom-elements.json, not
...
// web component's own render callback, then the `.map(fn)` that turns those nodes into
...
// web-component-meta.json / the CEM / llms-full.txt keep showing the AUTHORED text
...
// (the LIB-2 fix, see gen-web-component-types.mjs) — so a method signature naming
// `CardResolution` or `EntityRef` would be a bare TS2304 in the shipped
// dist/web-components.d.ts, invisible under the `skipLibCheck: true` every consumer
...
//
// `scope` is the WEB COMPONENT's own facade callback, not the file: resizable.tsx defines
    ...
    if (
      ...
      for (const prop of node.arguments[0].properties) {
        ...
        if (ts.isPropertyAssignment(prop) && (ts.isIdentifier(prop.name) || ts.isStringLiteralLike(prop.name))) {
          ...
          // absent from web-component-meta.json, custom-elements.json, web-component-types.d.ts,
          ...
          if (!init || !ts.isFunctionLike(init)) {
            throw new Error(
              `gen-web-component-api: expose({ ${name} }) in ${basename(sourceFile.fileName)} does not resolve to a ` +
...
for (const file of facadeFiles) {
  ...
  const visit = (node) => {
    if (
      ...
      // Scoped to THIS web component's render callback, not the file: source.tsx defines
      ...
      // per-web-component build (KAI_BUILD=split, config/vite/web-components.ts) names each
      // emitted module after its SOURCE FILE, not its tag, so the React wrappers must lazy-import
      // `@kitn.ai/ui/web-components/<module>` (not `<tag>`) to resolve a real dist file.
      // Non-enumerable so it stays out of the serialized web-component-meta.json.
...
// registry's pure-data shape: string/bool/number/array/object/identifier-ref.
{
  ...
  if (slotsSf) {
    ...
    // surface — 113 lines across web-component-meta.json, llms-full.txt and
    ...
    const evalNode = (node) => {
      ...
      if (ts.isArrayLiteralExpression(node)) {
        ...
        for (const el of node.elements) {
          ...
          if (!Array.isArray(spread)) {
            throw new Error(
              `gen-web-component-api (evalNode): a spread in ${REGISTRY} resolved to ${typeof spread}, not an\n` +
      ...
      if (ts.isIdentifier(node)) {
        ...
        if (!target) {
          throw new Error(
            `gen-web-component-api (evalNode): \`${node.text}\` is referenced by ${REGISTRY} but is not a\n` +
...
// its one "✓ dist/custom-elements.json" above and exited 0 while web-component-meta.json,
// icon-names.json, web-component-types.d.ts, the React wrappers, llms.txt/llms-full.txt and
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  // NAMED_ICONS object in src/components/icon/icon.tsx; extract its keys from the literal. ----
  {
    ...
    const names = block
      ...
      : [];
    writeFileSync(resolve(root, 'src/web-components/icon-names.json'), JSON.stringify(names, null, 2) + '\n');
    console.log(`✓ src/web-components/icon-names.json — ${names.length} curated icons`);
  }
  for (const mod of ['./gen-web-component-types.mjs', './gen-web-component-react.mjs']) {
    try {
