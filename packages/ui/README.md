...
The URLs above track the **latest** release — handy for trying things out. **For production, pin an exact version** (e.g. `@kitn.ai/ui@0.37.1/dist/kai.es.js`): pinned URLs are immutable and cached far more aggressively, and — since this package is pre-1.0 — pinning shields you from breaking changes in a future minor release.
...
The dev tooling ships as its own packages: the `kai` command line, `@kitn.ai/cli` (`npx -y @kitn.ai/cli add <block>`), and the MCP server, `@kitn.ai/mcp` — `npx -y @kitn.ai/mcp` gives any MCP harness tools to look up the real `kai-*` API, scaffold a wired chat surface, theme it, and catch the classic mistakes:
