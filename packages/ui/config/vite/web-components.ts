...
function registerConfig(): UserConfig {
  return {
    ...
    build: {
      lib: {
        entry: resolve(PKG, 'src/web-components/register.ts'),
        name: 'Kai',
...
function splitConfig(): UserConfig {
  ...
  // but it is also COMMITTED, and src/web-components/web-component-diagnostics.ts imports it
  ...
  const entry: Record<string, string> = {
    autoloader: resolve(PKG, 'src/web-components/autoloader.ts'),
    ...
    remote: resolve(PKG, 'src/web-components/remote.tsx'),
  };
  for (const file of Object.keys(manifest.files)) {
    for (const ext of ['tsx', 'ts']) {
      const p = resolve(PKG, `src/web-components/${file}.${ext}`);
      if (existsSync(p)) {
        entry[file] = p;
        break;
      }
