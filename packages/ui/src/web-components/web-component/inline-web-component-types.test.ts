...
const PKG_ROOT = resolve(HERE, '../../..'); // HERE is src/web-components/web-component/
...
describe('gen-web-component-types.mjs inline type block', () => {
  it('declares a ChatMessage structurally identical to src/web-components/chat/chat-types.ts', async () => {
  ...
  it('declares classifyTool with the real implementation signature', async () => {
    ...
    // (src/web-components/register/register.ts). The inline block DECLARES its signature by hand,
