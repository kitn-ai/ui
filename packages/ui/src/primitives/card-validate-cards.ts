// lint-comment-references: long-block -- the two tiers, the mirror requirement, the measured corpus and the strict-mode conclusion each have no other home
// Two-tier validation of a CardEnvelope's `data` against the card schema for its
// type, for the NATIVE dispatchers. "The card schema" is one of the seven built-ins,
// or a schema the app registered for a type of its own (`createCardRegistry`,
// src/schemas/registry.ts), passed as the optional third argument to `validateCardData`.
//
// THIS MIRRORS src/remote/provider-runtime.ts, DELIBERATELY. The remote transport has
// validated every incoming envelope since the card contract landed; the native path
// checked only that the TYPE was registered, so the same bad payload was caught in an
// iframe and rendered silently in-process. Change the failure shape here and there too.
//
// THE TIERS. HARD means the card has nothing to render (the field it is built around is
// absent, or a different kind of thing), so a diagnostic replaces the card. SOFT means
// the content is present and merely out of bounds, so the card renders and the failure
// is reported. Both emit `{ kind: 'error' }`; only hard changes what the user sees. Soft
// defaults ON by measurement: 35 recorded envelopes across five model configurations and
// both wires, zero trips. What the corpus does not cover (the structured-output path,
// live `artifact` cards, the constraints the spike clamps) is not claimed here; what the
// lean keyword subset cannot catch is per keyword in the generated `NOT_ENFORCED` table
// (`scripts/gen-card-validation-schemas.mjs`).
//
// WHY THIS IS ENFORCEMENT AND NOT A SAFETY NET. The card schemas cannot be emitted under
// either provider's strict mode today, so a tool definition guides the model rather than
// constraining it, and this check stands between a wrong shape and the user's screen.
...
/**
 * Keyword to tier: the line is PRESENCE versus BOUNDS, drawn per keyword rather
 * than per card because the dispatcher does not know what any given card component
 * does with a field. `hard` means nothing to draw; `soft` means it renders.
 *
 ...
 * The difference is what is lost: violating `confirm.actions`' `minItems: 1` means a
 * decision card with no decisions, so the visible result is empty chrome that reads
 * as a bug in this library, while one blank `actions[].label` still leaves the button
 * there, focusable, emitting the correct `id`.
 *
 * `type` is HARD everywhere, including on optional scalars where a truthy wrong type
 * would have rendered fine (`dismissible: "yes"`). Deliberately blunt: a per-field
 * carve-out would need the dispatcher to know which fields each card treats loosely,
 * and a wrong `type` is the strongest signal the model produced something else.
 */
