 ...
 *
 * ★ THIS IS NOT A LIST OF MEDIA TYPES. It used to be one in disguise
 * (`image | video | audio | document`, from a prefix switch) and it disagreed with
 * `wire/media-types.ts` in both directions: SVG and MP4 drew a convincing preview
 * that threw at encode time, while PDF and text, the formats the wire handles best,
 * drew the same anonymous icon as a `.zip`. `getMediaCategory` now asks the media
 * policy and returns what it says, so this is a projection of one declaration.
 *
 * `image` / `document` / `text` ARE `EncodableKind` from `wire/media-types.ts`,
 * spelled out rather than imported because this module is deliberately JSX-free and
 * dependency-free; the runtime agreement is pinned by
 * `attachment-media-policy.test.tsx`, whose drift guard derives both sides from the
 * policy. `video` and `audio` are GONE on purpose: no wire this kit ships can carry
 * either, so both now land in `unsendable` rather than making an unsendable file look
 * sendable.
 */
