...
| Entry | Use it for |
|---|---|
| `@kitn.ai/ui/elements` | Any framework — registers every `kc-*` element |
...
Import `@kitn.ai/ui/elements` once for its side effect. It registers every `kc-*` element; after that they work like built-in HTML.
