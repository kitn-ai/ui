...
kitn-chat is **authored in SolidJS** — the `kc-*` web components are compiled from the same components you import directly. Solid apps get two paths that freely mix:

- **Native SolidJS components** (`@kitn.ai/chat`) — import `ChatContainer`, `Message`, `PromptInput`, and the rest directly into your JSX. Full compositional control, tree-shaken alongside your own code. **This is the recommended path for Solid apps.**
