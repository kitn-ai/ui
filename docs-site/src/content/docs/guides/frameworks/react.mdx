...
`@kitn.ai/chat/react` exports a typed React wrapper for every `kc-*` web component. Pass data as props, handle events with `onX` handlers — no refs, no manual `addEventListener`, no stringified objects.
...
import '@kitn.ai/chat/elements'; // registers all kc-* elements globally
```

```tsx
// Any component file
import { Chat, Markdown, Reasoning } from '@kitn.ai/chat/react';
```
...
Every element is styled inside its own Shadow DOM. Import `@kitn.ai/chat/theme.css` only if you want to override design tokens.
</Aside>
...
```tsx
import { Markdown, Reasoning, Tool } from '@kitn.ai/chat/react';
...
`kc-image` exports as `Image`, which shadows the browser global. Alias it on import: `import { Image as KcImage } from '@kitn.ai/chat/react'`.
</Aside>
