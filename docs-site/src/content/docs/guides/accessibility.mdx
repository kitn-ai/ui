...
`auto` (the default) follows the OS `prefers-color-scheme` media query — users who enable dark mode at the OS level get it automatically.
...
5. **Use `theme="auto"`.** Forcing `theme="light"` on a user who prefers dark mode creates contrast mismatches between the kit and your host page. `auto` (the default) avoids this.
