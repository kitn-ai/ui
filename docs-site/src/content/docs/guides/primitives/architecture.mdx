...
description: The layered architecture behind the kc-* web components — headless primitives, UI primitives, feature components, and the Solid-to-custom-element facade.
---

import { Aside } from '@astrojs/starlight/components';

You don't need to know any of this to use `<kc-chat>`. Read it when you want to go deeper: import a building block directly, compose your own layout, or change the kit itself.

Every `kc-*` element is a thin Shadow-DOM wrapper around a SolidJS component. The interesting parts — the streaming, the auto-resize, the stick-to-bottom scroll, the accessible toggles — live in plain Solid code underneath. The web component is just the last mile that turns that code into a custom element any framework can use.
...
**4. The element facade** (`src/elements/`) — each feature component wrapped as a `kc-*` custom element. This layer adds Shadow DOM, the design-token stylesheet, prop/attribute handling, and `kc-*` CustomEvents. It's deliberately thin: most files are a few lines mapping props and events onto the component underneath.
...
src/elements/     →  kc-* custom elements (the Shadow-DOM facade)
```

The package exposes two of these layers directly. `@kitn.ai/chat` (the `.` export) gives you layers 1–3 as Solid source; `@kitn.ai/chat/elements` gives you layer 4, the registered `kc-*` elements. [Working with the components directly](/chat/guides/primitives/using-primitives/) covers the first; the rest of the docs cover the second.
...
`<kc-switch>` is a good small example — the whole element is the wrapper around the `Switch` UI primitive:

```tsx
import { defineWebComponent } from './define';
import { Switch } from '../ui/switch';

defineWebComponent('kc-switch', {
  checked: undefined,
  disabled: undefined,
  label: undefined,
}, (props, { dispatch, flag }) => (
...
- Hands the facade two helpers: `dispatch(type, detail)` fires a `kc-*` `CustomEvent` off the host, and `flag(name)` resolves a boolean the way HTML authors expect — `<el checked>`, `<el checked="true">`, and `el.checked = true` all read as on.
...
A UI primitive and its element are a pair: `Switch` (the Solid component) and `<kc-switch>` (the element). Only the elements are prefixed `kc-*`, because they claim global custom-element tag names; the Solid components are scoped imports, so they keep their plain names.
</Aside>

## Where to go next

- [Working with the components directly](/chat/guides/primitives/using-primitives/) — import the primitives and feature components into a Solid app.
- [Run the kit locally](/chat/guides/primitives/development/) — clone, build, and the Storybook dev loop.
- [Create or modify a component](/chat/guides/primitives/extending/) — add a primitive and expose it as a `kc-*` element.
