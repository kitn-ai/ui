...
```bash
git clone https://github.com/kitn-ai/chat.git
cd chat
npm install
```
...
Stories live next to their source as `*.stories.tsx`, in three places that mirror the [layers](/chat/guides/primitives/architecture/):

- `src/ui/*.stories.tsx` — UI primitives (`Button`, `Switch`, `Resizable`).
- `src/components/*.stories.tsx` — feature components (`Message`, `Artifact`).
- `src/elements/*.stories.tsx` — the `kc-*` web components.
...
The custom-element facade isn't testable under jsdom — it needs a real browser. So unit-test the underlying Solid component (e.g. `Switch`), not `<kc-switch>`. Element-level behavior that needs a browser is covered by Playwright instead.
</Aside>
...
- [Create or modify a component](/chat/guides/primitives/extending/) — add a primitive, give it a story, and expose it as a `kc-*` element.
