...
description: Add a UI primitive, give it a story, and expose it as a kc-* web component — the full path through kitn-chat's layers, with the Switch as a worked example.
---

import { Aside } from '@astrojs/starlight/components';

Adding a component means working down the [layers](/chat/guides/primitives/architecture/): write the Solid component, give it a story, then wrap it as a `kc-*` element. Modifying one is the same path in reverse — find the layer that owns the behavior and change it there. We'll build a toggle the whole way through; it's the shortest component that touches every step.
...
- **Boolean props** — use `flag('checked')`, not `props.checked`. A bare attribute (`<kc-switch checked>`) parses to `undefined`, so `flag` is what makes `<kc-switch checked>`, `checked="true"`, and `el.checked = true` all read as on.
- **Events** — `dispatch('kc-change', { checked })` fires a `kc-*` `CustomEvent` off the host. Typing `defineWebComponent<Props, Events>` keeps `dispatch` honest about names and payloads.
...
That's the full loop: the component exists, has a story, ships as `<kc-switch>`, and shows up everywhere the generated metadata feeds — the reference tables, the React adapter, and the for-AI-agents output.
