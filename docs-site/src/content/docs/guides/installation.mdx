...
Add `@kitn.ai/ui`, register the `kc-*` elements once, and use them in any framework. SolidJS is bundled in, so the host app needs nothing else.
...
|---|---|
| `@kitn.ai/ui/elements` | The web components — registers every `kc-*` element; use in any framework |
