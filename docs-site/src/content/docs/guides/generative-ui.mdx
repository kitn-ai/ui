...
For **provider-owned, cross-origin** cards you don't want to bundle, render them in a sandboxed iframe with `kai-remote` — same envelope, same policy, over `postMessage`. See [remote cards](/chat/examples/remote-cards/).
