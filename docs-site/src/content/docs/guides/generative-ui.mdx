...
This guide is the model/server side: the contract, how to get your model to emit cards, and how to feed the result back. For the rendering pattern see [Generative UI cards](/chat/patterns/generative-ui-cards/); for each card's props, the component pages ([confirm](/chat/components/confirm/), [choice](/chat/components/choice/), [form](/chat/components/form/), [tasks](/chat/components/tasks/)).

## The loop

```
agent / server ──CardEnvelope(s)──▶  <kc-cards>.cards
     ▲                                     │  dispatcher renders the kc-* for each `type`
     │                                     ▼
     └──── result ◀── CardPolicy ◀── kc-card event ◀── user interacts
```
...
|---|---|---|---|
| `confirm` | Approve / decline an action | `kc-confirm` | `action` |
| `choice` | Pick one option from a list | `kc-choice` | `action` |
| `form` | Collect structured input (JSON-Schema form) | `kc-form` | `submit` |
| `tasks` | Select from a proposed plan | `kc-tasks` | `submit` |
| `link` | Preview a URL (title/desc/image) | `kc-link-preview` | — |
| `embed` | Lazy media embed (YouTube, Vimeo) | `kc-embed` | — |
...
```ts
import { validateAgainstSchema } from '@kitn.ai/chat';
...
Render envelopes with `<kc-cards>` and route every interaction through a `CardPolicy`:
...
cards.addEventListener('kc-card', (e) => {
...
```ts
import { mergeCardTags } from '@kitn.ai/chat';
const types = mergeCardTags({ chart: 'my-chart-card' }); // built-ins + yours
// el.types = types  →  <kc-cards> now renders `chart` envelopes with <my-chart-card>
```

For **provider-owned, cross-origin** cards you don't want to bundle, render them in a sandboxed iframe with `kc-remote` — same envelope, same policy, over `postMessage`. See [remote cards](/chat/examples/remote-cards/).

## Next

- [Generative UI cards](/chat/patterns/generative-ui-cards/) — the rendering pattern, live.
- [confirm](/chat/components/confirm/) · [choice](/chat/components/choice/) · [form](/chat/components/form/) · [tasks](/chat/components/tasks/) — per-card APIs.
