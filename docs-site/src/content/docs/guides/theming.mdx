...
```tsx
import { ChatConfig } from '@kitn.ai/chat';
...
The full list of overridable tokens lives in `theme.css` (shipped at `@kitn.ai/chat/theme.css`). For a browser-ready version with `:root` and `.dark` blocks pre-separated, use `@kitn.ai/chat/theme.tokens.css` — it is auto-generated from `theme.css` and never drifts.
