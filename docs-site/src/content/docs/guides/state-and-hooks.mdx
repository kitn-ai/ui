...
// Patch a field on an existing message (e.g. set loading: false when done)
setMessages((m) => updateMessage(m, id, { loading: false }));
...
You can pass an optional `init` object as the second argument to seed the message with an id,
model label, or any other fields before the stream starts:

```ts
const s = createAssistantStream(setMessages, { model: 'claude-sonnet-4' });
```
