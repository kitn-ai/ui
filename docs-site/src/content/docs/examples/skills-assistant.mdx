...
- **[`kai-chat` reference](/chat/components/chat/)** — the `slashCommands`, `slashActiveIds`, and `slashCompact` props plus the `kai-slash-select` and `kai-submit` events.
- **[`kai-skills` reference](/chat/components/skills/)** — the `skills` property and the declarative `<kai-skill>` children path.
