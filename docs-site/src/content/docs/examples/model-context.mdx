...
- **[`kai-chat` reference](/chat/components/chat/)** — every header prop, including `models`, `currentModel`, and `context`.
- **[`kai-model-switcher` reference](/chat/components/model-switcher/)** — the standalone switcher with a `models` property or declarative `<kai-model>` children.
- **[`kai-context` reference](/chat/components/context/)** — the standalone meter, configurable `warnThreshold` / `dangerThreshold`, and the `kai-threshold-change` event.
