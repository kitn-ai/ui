---
title: Custom theme
description: Reskin <kai-chat> into a bold, distinctive brand — magenta accent, cyan focus glow, deep-noir surfaces — using only inherited --kai-* tokens, in both light and dark.
---

import CustomThemeShowcaseDemo from '../../../components/CustomThemeShowcaseDemo.tsx';

<p class="kai-lede">This is the **same `<kai-chat>`** that powers every other example, reskinned into "Aurora" — a neon-on-noir identity with a magenta accent, a cyan focus ring, and sharp corners. Nothing pierces the Shadow DOM: a map of inherited `--kai-*` custom properties on the host does all of it. Toggle the site's theme to watch a second, matched palette take over.</p>
...
- **[Theming guide](/chat/guides/theming/)** — the full token reference, the `--kai-color-*` alias system, scoped subtree theming, and dark-mode setup.
