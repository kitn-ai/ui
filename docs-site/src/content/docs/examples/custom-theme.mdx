---
title: Custom theme
description: Reskin <kai-chat> into a bold, distinctive brand — magenta accent, cyan focus glow, deep-noir surfaces — using only inherited --kc-* tokens, in both light and dark.
---

import CustomThemeShowcaseDemo from '../../../components/CustomThemeShowcaseDemo.tsx';

<p class="kc-lede">This is the **same `<kai-chat>`** that powers every other example, reskinned into "Aurora" — a neon-on-noir identity with a magenta accent, a cyan focus ring, and sharp corners. Nothing pierces the Shadow DOM: a map of inherited `--kc-*` custom properties on the host does all of it. Toggle the site's theme to watch a second, matched palette take over.</p>

<CustomThemeShowcaseDemo client:only="solid" />

## How it works

The `--kc-color-*` and `--kc-radius` tokens are **inherited** CSS custom properties. Set them on the `<kai-chat>` element (or any ancestor) and they propagate into the Shadow DOM through `var()` fallbacks — no stylesheet import, no internal classes touched.
...
- **[Drop-in chat](/chat/examples/drop-in-chat/)** — the streaming loop that backs any `<kai-chat>` instance.
- **[`kai-chat` reference](/chat/components/chat/)** — `proseSize`, `codeTheme`, and `codeHighlight` for non-color appearance control.
