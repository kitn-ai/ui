...
- **[Theming guide](/guides/theming/)** — the full token reference, the `--kai-color-*` alias system, scoped subtree theming, and dark-mode setup.
- **[Drop-in chat](/examples/drop-in-chat/)** — the streaming loop that backs any `<kai-chat>` instance.
- **[`kai-chat` reference](/components/chat/)** — `proseSize`, `codeTheme`, and `codeHighlight` for non-color appearance control.
