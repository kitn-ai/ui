...
- **[`kai-scope-picker` reference](/chat/components/scope-picker/)** — `availableAuthors`, `availableTags`, `currentLabel`, and the `kai-scope-change` event.
- **[`kai-conversations` reference](/chat/components/conversations/)** — grouping, `activeId`, and the `kai-conversation-select` event.
