...
<p class="kai-lede">Mount a card from an external provider in a sandboxed cross-origin iframe — it speaks the same <code>CardEnvelope</code> / <code>CardEvent</code> contract as every built-in card, so your routing policy and event listeners work without change.</p>
