...
| `duration` | `number` (ms) | `2000` | Auto-dismiss delay. `0` is sticky. With an `action`, the floor is `4000` so there's time to act. |
...
A confirmation with the emerald check. Auto-dismisses after 2 seconds.
...
Add an `action` to offer a one-tap reversal. The toast stays up at least 4 seconds when it carries an action, so the user has time to react:
...
| `duration` | `number` (ms) | `0` = sticky; defaults to `2000` (floor `4000` with an action). |
| `dismissible` | `boolean` | Show the × button. Defaults to `true`. |

## Behavior

- **Auto-dismiss** after `duration` (2s default), **paused while hovered** so it doesn't vanish mid-read.
