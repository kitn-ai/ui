...
<p class="kai-lede">`<kai-prompt-input>` ships with an attach button. Click the paperclip, pick a file, send — `kai-submit` delivers `{ value, attachments }` together, and the user's `<kai-message>` renders the files inline. You own the upload logic; kitn-chat handles the staging and display.</p>
