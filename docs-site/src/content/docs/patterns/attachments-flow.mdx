...
Pass `attachments` in the `ChatMessage` object. `<kai-chat>` and `<kai-message>` render them inline beneath the message text — the same display the [`<kai-attachments>`](/components/attachments/) element uses — with no extra wiring:
...
- **[`kai-message` reference](/components/message/)** — full `ChatMessage` shape including `attachments`.
