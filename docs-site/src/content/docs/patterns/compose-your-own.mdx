...
- **[Drop-in chat](/chat/examples/drop-in-chat/)** — the 90% path: one `<kai-chat>` element with no manual wiring.
- **[Streaming recipe](/chat/guides/recipes/streaming/)** — wire the `kai-submit` loop to a real SSE stream.
- **[`kai-resizable` reference](/chat/components/resizable/)** — `orientation`, `maximizedIndex`, `kai-change`.
- **[`kai-conversations` reference](/chat/components/conversations/)** — `groups` vs `conversations`, `activeId`, events.
- **[`kai-message` reference](/chat/components/message/)** — `message` shape, `actionsReveal`, `proseSize`.
- **[`kai-prompt-input` reference](/chat/components/prompt-input/)** — `loading`, `stoppable`, `slashCommands`, `kai-toolbar-action`.
