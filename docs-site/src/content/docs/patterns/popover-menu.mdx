...
<p class="kai-lede">A trigger that opens a floating card — a model picker, a settings menu, an account dropdown. `<kai-popover>` handles the open/close, positioning, and dismissal; you slot the trigger and whatever content the card needs. The menu below rebuilds the ChatGPT header popover: a flagship model row, an expandable group, and a toggle.</p>
