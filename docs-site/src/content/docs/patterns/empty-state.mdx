...
<p class="kc-lede">A new user lands on a blank thread — give them somewhere to start. Compose a `<kai-empty>` block (icon, headline, subtext) with `<kai-suggestions>` chips **inside** it, so the greeting and the starter prompts read as one piece. Click a chip below to start the conversation.</p>
