...
<p class="kai-lede">A new user lands on a blank thread — give them somewhere to start. Compose a `<kai-empty>` block (icon, headline, subtext) with `<kai-suggestions>` chips **inside** it, so the greeting and the starter prompts read as one piece. Click a chip below to start the conversation.</p>
...
- **[`kai-chat` reference](/chat/components/chat/)** — full prop and event list, including `suggestions`, `suggestionMode`, `placeholder`, and `chatTitle`.
- **[`kai-empty` reference](/chat/components/empty/)** — props and slots for the empty-state block.
- **[`kai-suggestions` reference](/chat/components/suggestions/)** — standalone suggestion chips for custom layouts outside `<kai-chat>`.
