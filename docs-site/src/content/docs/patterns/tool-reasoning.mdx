...
Both `tools` and `reasoning` live directly on the `ChatMessage` object you pass to `<kc-chat>` or `<kc-message>`. Update the message in place as the agent streams. Since the message carries arrays, you assign it in JavaScript rather than as an HTML attribute.
