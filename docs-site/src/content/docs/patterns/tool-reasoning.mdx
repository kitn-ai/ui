...
<p class="kai-lede">Agents think before they act and invoke tools to get things done. The `tools` and `reasoning` fields on `ChatMessage` bring both inline — one object per tool call, updated in place as the agent streams its output.</p>
...
- **[Drop-in chat](/chat/examples/drop-in-chat/)** — the full `kai-submit` streaming loop.
- **[`kai-message` reference](/chat/components/message/)** — complete `ChatMessage` shape, `actionsReveal`, `proseSize`.
- **[`kai-tool` reference](/chat/components/tool/)** — `tool` property shape, `open` flag, state tokens.
- **[`kai-reasoning` reference](/chat/components/reasoning/)** — `streaming` auto-expand, `kai-open-change`.
- **[`kai-thinking-bar` reference](/chat/components/thinking-bar/)** — `text`, `stoppable`, `kai-stop`.
- **[Compose your own shell](/chat/patterns/compose-your-own/)** — lay out the thread and composer without `<kai-chat>`.
