...
- **[`kai-cards` reference](/chat/components/cards/)** — `types` map for custom card tags, all props.
- **[`kai-confirm` reference](/chat/components/confirm/)** — `autofocus`, `tone`, full action shapes.
- **[`kai-choice` reference](/chat/components/choice/)** — `allowOther`, media options.
- **[`kai-form` reference](/chat/components/form/)** — full JSON Schema subset + every `x-kc-*` hint.
- **[Drop-in chat](/chat/examples/drop-in-chat/)** — wire cards into a streaming conversation with `<kai-chat>`.
