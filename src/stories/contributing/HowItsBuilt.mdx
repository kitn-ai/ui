...
<Meta title="Contributing/How it's built" />
...
The package exposes two of these layers directly. `@kitn.ai/ui` (the `.` export) gives you layers 1–3 as Solid source; `@kitn.ai/ui/elements` gives you layer 4, the registered `kai-*` elements. The **Contributing → Working with the components directly** page covers the first; the rest of the docs cover the second.
...
> **Note — two names for the same thing.** A UI primitive and its element are a pair: `Switch` (the Solid component) and `<kai-switch>` (the element). Only the elements are prefixed `kai-*`, because they claim global custom-element tag names; the Solid components are scoped imports, so they keep their plain names. See the rendered [`<kai-switch>` element](https://ui.kitn.ai/components/switch/) in the public docs.

## Where to go next

- **Contributing → Working with the components directly** — import the primitives and feature components into a Solid app.
- **Contributing → Run the kit locally** — clone, build, and the Storybook dev loop.
- **Contributing → Create or modify a component** — add a primitive and expose it as a `kai-*` element.
