...
The package exposes two of these layers directly. `@kitn.ai/ui` (the `.` export) gives you layers 1–3 as Solid source; `@kitn.ai/ui/elements` gives you layer 4, the registered `kai-*` elements. The **Getting Started → Working with the primitives** page covers the first; the rest of the docs cover the second.
...
- **Getting Started → Working with the primitives** — import the primitives and feature components into a Solid app.
- **Getting Started → Run the kit locally** — clone, build, and the Storybook dev loop.
- **Getting Started → Create or modify a component** — add a primitive and expose it as a `kai-*` element.
