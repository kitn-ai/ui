...
> **Note — reserved prop names.** `title`, `id`, `slot`, and `lang` collide with global HTMLElement attributes and break the element constructor — `defineWebComponent` throws if you use them. Rename (e.g. a source title → `headline`).
...
> **Keep the layers honest.** Behavior belongs in `src/primitives/`, accessible markup in `src/ui/`, the AI surface in `src/components/`, and only the thin wrapper in `src/elements/`. When you modify a component, change the layer that owns the behavior — the element facade should stay boring.
