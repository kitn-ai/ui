...
<script type="module">
  ...
  sidebar.addEventListener('conversationselect', (e) => {
    chat.messages = loadMessages(e.detail.id);
  });

  sidebar.addEventListener('newchat', () => startNewConversation());

  chat.addEventListener('submit', (e) => sendMessage(e.detail.value));
</script>
```
...
el.addEventListener('submit', (e) => console.log(e.detail.value));
el.addEventListener('conversationselect', (e) => console.log(e.detail.id));
```
