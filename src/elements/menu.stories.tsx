...
// ---------------------------------------------------------------------------
// Dark — validates that kai-menu renders correctly with theme="dark"
// ---------------------------------------------------------------------------

function PlusMenuDarkDemo() {
  let el: MenuEl | undefined;
  ...
  onMount(() => {
    ...
    el.addEventListener('kai-select', (e) => {
      const detail = (e as CustomEvent<{ id: string; checked?: boolean }>).detail;
      console.log('[kai-menu dark] kai-select', detail);
  ...
  return (
    <div style={{ padding: '48px', background: '#161618', 'min-height': '100vh', display: 'flex', gap: '16px', 'align-items': 'flex-start' }}>
      <kai-menu ref={(e) => (el = e as MenuEl)} theme="dark">
        <span slot="trigger" style={{ display: 'flex', 'align-items': 'center', 'justify-content': 'center' }}>
          <Plus size={18} />
        </span>
      </kai-menu>
      <p style={{ 'font-size': '13px', color: '#71717a', 'margin-top': '2px' }}>
        Dark mode. Click the + icon to open the menu. Watch the console for <code>kai-select</code> events.
      </p>
    </div>
  );
}
