...
declare module 'solid-js' {
  ...
  namespace JSX {
    interface IntrinsicElements {
      'kai-popover': JSX.HTMLAttributes<HTMLElement> & { placement?: string; gutter?: number | string; open?: boolean };
    }
  }
}

const HTML_SNIPPET = `<kai-popover placement="bottom-start">
...
const meta = {
  ...
  tags: ['autodocs'],
  argTypes: argTypesFor('kai-popover'),
  parameters: {
    layout: 'centered',
    docs: {
      description: specDescription('kai-popover', [
        '`<kai-popover>` is the framework-agnostic **web component** for a button-and-popover card — a trigger that toggles a floating panel of arbitrary content, isolated in **Shadow DOM**. In SolidJS, use the `Popover` primitive.',
        ...
        '**How to use:** slot the control as `slot="trigger"` and the panel as the default slot. Clicking the trigger toggles; Escape or an outside click closes (clicks inside do not). It fires `kai-open-change` with `{ open }` and accepts an `open` property for controlled use.',
...
export const ModelMenu: Story = {
  render: () => {
    ...
    onMount(() => el?.addEventListener('kai-open-change', (e) => console.log('kai-open-change', (e as CustomEvent<{ open: boolean }>).detail.open)));
    return (
      <kai-popover ref={(e) => (el = e as HTMLElement)} placement="bottom-start">
